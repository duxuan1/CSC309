<?php
	...
	require_once "model/getProfile.php";
	...
	switch($_SESSION['state']){
		...
		case "register":
			...
			} else if ($row = !pg_fetch_array($result, NULL, PGSQL_ASSOC)) {
				$email = $_REQUEST["email"];
				$fav = $_POST["favourite"];
				$gender = $_POST["gender"];
				// insert to database
				$insert_user_query="INSERT INTO appuser (userid, password, email, gender, favourite, guesssolved, rpssolved, frogssolved) VALUES ($1,$2,$3,$4,$5,$6,$7,$8);";
				$result = pg_prepare($dbconn, "", $insert_user_query);
				$result = pg_execute($dbconn, "", array($_REQUEST['user'], $_REQUEST['password'], $email, $gender, $fav, 0, 0, 0)); 
				$errors[]="success registe!";
			}
			break;
		...
		case "profile": 
			...
			// $query="SELECT userid, password, email, gender, favourite FROM appuser WHERE userid = '" . $_SESSION['user'] . "';";
			...
			$_SESSION["profile"]->get_profile($dbconn);
